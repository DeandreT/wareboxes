...
use wareboxes_core::models::{
    InventoryRelocationClaimHeartbeat, InventoryRelocationClaimRelease,
    InventoryRelocationClaimReleaseReason, TenantAccess, Timestamp,
};
...
const HEARTBEAT_OPERATION: &str = "inventory_relocation.heartbeat.v1";
const RELEASE_OPERATION: &str = "inventory_relocation.release.v1";
const MAX_RELEASE_NOTE_LENGTH: usize = 500;

struct LockedRelocationClaim {
...
async fn lock_active_relocation_claim_tx(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    access: &TenantAccess,
    task_id: i64,
    scope: &ScopeBindings,
) -> AppResult<LockedRelocationClaim> {
    let row = sqlx::query(
        r#"
        SELECT task.status, task.assigned_user_id, task.lease_expires_at,
               task.lease_expires_at > statement_timestamp() AS lease_is_current,
               task.task_timeout_seconds, task.facility_id,
        ...
        || !row.try_get::<bool, _>("detail_is_open")?
    {
        return Err(AppError::conflict(
            "inventory relocation claim is no longer active",
        ));
    }
    let inventory_owner_id = InventoryOwnerId::new(
        dimensions
            .inventory_owner_id
            .ok_or_else(|| AppError::internal("relocation task has no inventory owner"))?,
    )
    .map_err(|error| AppError::internal(error.to_string()))?;
    Ok(LockedRelocationClaim {
        inventory_owner_id,
        facility_id: dimensions
            .facility_id
            .ok_or_else(|| AppError::internal("relocation task has no facility"))?,
        lease_expires_at: row
            .try_get::<Option<Timestamp>, _>("lease_expires_at")?
            .ok_or_else(|| AppError::conflict("relocation claim has no active lease"))?,
        task_timeout_seconds: row.try_get("task_timeout_seconds")?,
    })
}

async fn require_relocation_claim_visible_tx(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    access: &TenantAccess,
    task_id: i64,
    scope: &ScopeBindings,
    ...
    if dimensions.is_allowed_by(scope) {
        ...
        Err(AppError::not_found("inventory relocation claim"))
    }
}

fn validate_release_input(
    reason: InventoryRelocationClaimReleaseReason,
    note: Option<&str>,
) -> AppResult<()> {
    ...
    if reason == InventoryRelocationClaimReleaseReason::Other && note.is_none() {
        return Err(AppError::bad_request(
...
pub async fn heartbeat_inventory_relocation_claim_in_scope(
    db: &Db,
    access: &TenantAccess,
    command: &CommandContext,
    task_id: i64,
) -> AppResult<InventoryRelocationClaimHeartbeat> {
    ...
    if let Some(heartbeat) = prepared
        .replayed::<InventoryRelocationClaimHeartbeat>(&mut tx)
        .await?
    {
        require_relocation_claim_visible_tx(&mut tx, access, task_id, &scope).await?;
        tx.commit().await?;
        return Ok(heartbeat);
    }

    let claim = lock_active_relocation_claim_tx(&mut tx, access, task_id, &scope).await?;
    ...
    .fetch_optional(&mut *tx)
    .await?
    .ok_or_else(|| AppError::conflict("inventory relocation claim is no longer active"))?;
    ...
    .await?;
    let heartbeat = InventoryRelocationClaimHeartbeat {
    ...
    Ok(prepared.commit(tx, heartbeat).await?)
}

pub async fn release_inventory_relocation_claim_in_scope(
    db: &Db,
    access: &TenantAccess,
    command: &CommandContext,
    task_id: i64,
    reason: InventoryRelocationClaimReleaseReason,
    note: Option<&str>,
) -> AppResult<InventoryRelocationClaimRelease> {
    command.require_actor(access.tenant_id, access.user_id)?;
    if task_id <= 0 {
        return Err(AppError::bad_request(
            "inventory relocation task ID must be positive",
    ...
    bind_tenant_context(&mut tx, access.tenant_id).await?;
    let scope = lock_current_scope_tx(&mut tx, access.tenant_id, command.actor_id.get()).await?;
    if let Some(release) = prepared
        .replayed::<InventoryRelocationClaimRelease>(&mut tx)
        .await?
    {
        require_relocation_claim_visible_tx(&mut tx, access, task_id, &scope).await?;
        tx.commit().await?;
    ...
    .await?;
    let release = InventoryRelocationClaimRelease {
        tenant_id: access.tenant_id,
        task_id,
        inventory_owner_id: claim.inventory_owner_id,
        facility_id: claim.facility_id,
    ...
    Ok(prepared.commit(tx, release).await?)
}
