    ...
    let sql = format!(
        ...
        WHERE tenant_id = $1
          ...
          AND (
              ...
              OR EXISTS (SELECT 1 FROM license_plate_putaway_tasks d WHERE d.tenant_id = work_tasks.tenant_id AND d.task_id = work_tasks.id AND (d.source_location_id = $6 OR d.destination_location_id = $6))
          )
          AND (
              $7::BIGINT IS NULL
