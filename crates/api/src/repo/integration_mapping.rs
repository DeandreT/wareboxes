...
use wareboxes_application::integration_mapping::{
    ...
    ConfigureIntegrationOrderOwnerMappingCommand, ConfigureIntegrationOrderOwnerMappingResult,
    ...
    IntegrationOrderOwnerMappingCursor, IntegrationOrderOwnerMappingPage,
    ...
    RetireIntegrationOrderOwnerMappingCommand, RetireIntegrationOrderOwnerMappingResult,
    CONFIGURE_INTEGRATION_ORDER_ITEM_MAPPING_OPERATION,
    CONFIGURE_INTEGRATION_ORDER_OWNER_MAPPING_OPERATION,
    RETIRE_INTEGRATION_ORDER_ITEM_MAPPING_OPERATION,
    RETIRE_INTEGRATION_ORDER_OWNER_MAPPING_OPERATION,
};
...
pub(crate) fn owner_natural_lock_key(
    tenant_id: TenantId,
    source_key: &str,
    external_inventory_owner_key: &str,
) -> String {
    format!(
        ...
        source_key.len(),
        source_key,
        external_inventory_owner_key.len(),
        external_inventory_owner_key,
    )
}

const OWNER_MAPPING_SELECT: &str = r#"
    SELECT mapping.*,owner.name AS inventory_owner_name
...
fn owner_status_from(retired_at: Option<Timestamp>) -> IntegrationOrderOwnerMappingStatus {
    if retired_at.is_some() {
...
fn map_owner_row(row: &sqlx::postgres::PgRow) -> AppResult<IntegrationOrderOwnerMappingReadModel> {
    let retired_at = row.try_get("effective_to")?;
    Ok(IntegrationOrderOwnerMappingReadModel {
        mapping_id: IntegrationOrderOwnerMappingId::new(row.try_get("id")?).map_err(internal)?,
        inventory_owner_name: row.try_get("inventory_owner_name")?,
        definition: IntegrationOrderOwnerMappingDefinition {
            ...
            external_inventory_owner_key: ExternalInventoryOwnerKey::new(
                row.try_get::<String, _>("external_inventory_owner_key")?,
            )
            .map_err(internal)?,
            inventory_owner_id: InventoryOwnerId::new(row.try_get("inventory_owner_id")?)
                .map_err(internal)?,
        },
        status: owner_status_from(retired_at),
        revision: IntegrationOrderOwnerMappingRevision::new(row.try_get("revision")?)
            .map_err(internal)?,
        configured_by: UserId::new(row.try_get("configured_by_user_id")?).map_err(internal)?,
        configured_at: row.try_get("configured_at")?,
        retired_by: row
...
async fn require_owner_replay_visible_tx(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    prepared: &PreparedCommand,
    scope: &ScopeBindings,
) -> AppResult<()> {
    let owner_id = sqlx::query_scalar::<_, i64>(
        ...
        SELECT mapping.inventory_owner_id
        FROM command_idempotency_records command
        JOIN integration_order_owner_mappings mapping
          ON mapping.tenant_id=command.tenant_id
         AND mapping.id=(command.result_json->>'mapping_id')::BIGINT
        WHERE command.tenant_id=$1 AND command.operation=$2 AND command.idempotency_key=$3
        "#,
...
async fn lock_owner_natural_key_tx(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    definition: &IntegrationOrderOwnerMappingDefinition,
) -> AppResult<()> {
    sqlx::query("SELECT pg_advisory_xact_lock(hashtextextended($1,0))")
        .bind(owner_natural_lock_key(
            definition.tenant_id,
            definition.source_key.as_str(),
            definition.external_inventory_owner_key.as_str(),
        ))
        .execute(&mut **tx)
        .await?;
    Ok(())
...
async fn require_owner_target_tx(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    definition: &IntegrationOrderOwnerMappingDefinition,
) -> AppResult<()> {
    let exists = sqlx::query_scalar::<_, bool>(
        r#"
        SELECT TRUE FROM inventory_owners owner
    ...
    if exists {
        ...
        Err(AppError::not_found("active inventory owner"))
    }
}

async fn latest_owner_mapping_tx(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    definition: &IntegrationOrderOwnerMappingDefinition,
) -> AppResult<
    Option<(
        IntegrationOrderOwnerMappingId,
        IntegrationOrderOwnerMappingRevision,
        InventoryOwnerId,
        bool,
    )>,
> {
    let row = sqlx::query(
        r#"
        SELECT id,revision,inventory_owner_id,effective_to IS NULL AS active
        FROM integration_order_owner_mappings
        WHERE tenant_id=$1 AND source_key=$2 AND external_inventory_owner_key=$3
        ORDER BY revision DESC LIMIT 1 FOR UPDATE
        "#,
    )
    .bind(definition.tenant_id.get())
    .bind(definition.source_key.as_str())
    .bind(definition.external_inventory_owner_key.as_str())
    .fetch_optional(&mut **tx)
    .await?;
    row.map(|row| {
        Ok((
            IntegrationOrderOwnerMappingId::new(row.try_get("id")?).map_err(internal)?,
            IntegrationOrderOwnerMappingRevision::new(row.try_get("revision")?)
                .map_err(internal)?,
...
async fn retire_owner_row_tx(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    tenant_id: TenantId,
    mapping_id: IntegrationOrderOwnerMappingId,
    actor_id: UserId,
    retired_at: Timestamp,
) -> AppResult<()> {
    sqlx::query(
        "UPDATE integration_order_owner_mappings SET effective_to=$3,retired_by_user_id=$4 WHERE tenant_id=$1 AND id=$2 AND effective_to IS NULL",
    )
    .bind(tenant_id.get())
    .bind(mapping_id.get())
    .bind(retired_at)
...
async fn insert_owner_mapping_tx(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    command: &ConfigureIntegrationOrderOwnerMappingCommand,
    revision: IntegrationOrderOwnerMappingRevision,
    predecessor: Option<IntegrationOrderOwnerMappingId>,
    actor_id: UserId,
    configured_at: Timestamp,
) -> AppResult<IntegrationOrderOwnerMappingId> {
    let definition = &command.definition;
    let id = sqlx::query_scalar::<_, i64>(
        r#"
        INSERT INTO integration_order_owner_mappings
            (tenant_id,source_key,external_inventory_owner_key,inventory_owner_id,
             revision,supersedes_mapping_id,effective_from,configured_by_user_id,configured_at)
    ...
    .bind(definition.inventory_owner_id.get())
    .bind(revision.get())
    .bind(predecessor.map(IntegrationOrderOwnerMappingId::get))
    .bind(configured_at)
    .bind(actor_id.get())
    .fetch_one(&mut **tx)
    .await?;
    IntegrationOrderOwnerMappingId::new(id).map_err(internal)
}

async fn read_owner_mapping_tx(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    tenant_id: TenantId,
    mapping_id: IntegrationOrderOwnerMappingId,
) -> AppResult<IntegrationOrderOwnerMappingReadModel> {
    let query = format!("{OWNER_MAPPING_SELECT} WHERE mapping.tenant_id=$1 AND mapping.id=$2");
    sqlx::query(&query)
        .bind(tenant_id.get())
        .bind(mapping_id.get())
        .fetch_optional(&mut **tx)
        ...
        .map(map_owner_row)
        .transpose()?
        .ok_or_else(|| AppError::not_found("integration order owner mapping"))
}

async fn enqueue_owner_event_tx(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    result: &IntegrationOrderOwnerMappingReadModel,
    actor_id: UserId,
    transition: &str,
    occurred_at: Timestamp,
) -> AppResult<()> {
    ...
    let ordering_key = format!(
        ...
        definition.source_key.as_str().len(),
        definition.source_key,
        definition.external_inventory_owner_key.as_str().len(),
        definition.external_inventory_owner_key,
    );
    let sequence = next_outbox_sequence_tx(tx, definition.tenant_id, &ordering_key).await?;
    let event_type = format!("integration.order_owner_mapping.{transition}");
    let event_key = format!("{ordering_key}:{}:{transition}", result.revision.get());
    let aggregate_id = result.mapping_id.get().to_string();
    let payload = serde_json::to_value(result).map_err(internal)?;
    outbox::enqueue(
        tx,
        &NewOutboxEvent {
            tenant_id: definition.tenant_id,
            inventory_owner_id: Some(definition.inventory_owner_id),
            facility_id: None,
            actor_user_id: Some(actor_id.get()),
            event_key: &event_key,
            aggregate_type: "integration_order_owner_mapping",
            aggregate_id: &aggregate_id,
            ordering_key: &ordering_key,
            aggregate_sequence: sequence,
            event_type: &event_type,
...
pub async fn configure_owner(
    ...
    context: &CommandContext,
    command: &ConfigureIntegrationOrderOwnerMappingCommand,
) -> AppResult<ConfigureIntegrationOrderOwnerMappingResult> {
    context.require_actor(access.tenant_id, access.user_id)?;
    if command.definition.tenant_id != access.tenant_id {
        return Err(AppError::not_found("integration order owner mapping"));
    }
    let prepared = PreparedCommand::new_v1(
        context,
        CONFIGURE_INTEGRATION_ORDER_OWNER_MAPPING_OPERATION,
    ...
    .await?;
    require_owner_replay_visible_tx(&mut tx, &prepared, &scope).await?;
    require_owner_scope(&scope, command.definition.inventory_owner_id.get())?;
    if let Some(result) = prepared.replayed(&mut tx).await? {
        tx.commit().await?;
        return Ok(result);
    }
    lock_owner_natural_key_tx(&mut tx, &command.definition).await?;
    let predecessor = latest_owner_mapping_tx(&mut tx, &command.definition).await?;
    if let Some((_, _, predecessor_owner, _)) = predecessor {
        require_owner_scope(&scope, predecessor_owner.get())?;
    }
    match (predecessor, command.expected_revision) {
        (None, None) | (Some((_, _, _, false)), None) => {}
        (Some((_, revision, _, true)), Some(expected)) if revision == expected => {}
        (Some((_, _, _, true)), None) => {
            return Err(AppError::conflict(
                "integration order owner mapping already exists",
            ));
        }
        (None, Some(_)) | (Some((_, _, _, false)), Some(_)) => {
            return Err(AppError::conflict(
                "integration order owner mapping has no active revision",
            ));
        }
        (Some((_, _, _, true)), Some(_)) => {
            return Err(AppError::conflict(
                "integration order owner mapping revision is stale",
            ));
        }
    }
    require_owner_target_tx(&mut tx, &command.definition).await?;
    let configured_at = now_iso();
    let retired_predecessor = if let Some((id, _, _, true)) = predecessor {
        retire_owner_row_tx(
            &mut tx,
            access.tenant_id,
            id,
        ...
        .await?;
        Some(read_owner_mapping_tx(&mut tx, access.tenant_id, id).await?)
    } else {
        None
    };
    let revision = match predecessor {
        Some((_, revision, _, _)) => revision
            .checked_next()
            .ok_or_else(|| AppError::internal("integration owner mapping revision overflow"))?,
        None => IntegrationOrderOwnerMappingRevision::new(1).map_err(internal)?,
    };
    let mapping_id = insert_owner_mapping_tx(
        &mut tx,
        command,
        revision,
        predecessor.map(|(id, _, _, _)| id),
        context.actor_id,
        configured_at,
    )
    .await?;
    let result = read_owner_mapping_tx(&mut tx, access.tenant_id, mapping_id).await?;
    if let Some(retired_predecessor) = retired_predecessor {
...
pub async fn retire_owner(
    ...
    access: &TenantAccess,
    context: &CommandContext,
    command: &RetireIntegrationOrderOwnerMappingCommand,
) -> AppResult<RetireIntegrationOrderOwnerMappingResult> {
    context.require_actor(access.tenant_id, access.user_id)?;
    let prepared = PreparedCommand::new_v1(
        context,
        RETIRE_INTEGRATION_ORDER_OWNER_MAPPING_OPERATION,
    ...
    let hint = sqlx::query(
        r#"
        SELECT source_key,external_inventory_owner_key,inventory_owner_id
        FROM integration_order_owner_mappings WHERE tenant_id=$1 AND id=$2
        "#,
    )
    .bind(access.tenant_id.get())
    .bind(command.mapping_id.get())
    ...
    .await?
    .ok_or_else(|| AppError::not_found("integration order owner mapping"))?;
    let definition = IntegrationOrderOwnerMappingDefinition {
        tenant_id: access.tenant_id,
        source_key: IntegrationSourceKey::new(hint.try_get::<String, _>("source_key")?)
            .map_err(internal)?,
        external_inventory_owner_key: ExternalInventoryOwnerKey::new(
            hint.try_get::<String, _>("external_inventory_owner_key")?,
        )
        .map_err(internal)?,
        inventory_owner_id: InventoryOwnerId::new(hint.try_get("inventory_owner_id")?)
            .map_err(internal)?,
    ...
    require_owner_scope(&scope, definition.inventory_owner_id.get())?;
    lock_owner_natural_key_tx(&mut tx, &definition).await?;
    let row = sqlx::query(
        "SELECT revision,effective_to FROM integration_order_owner_mappings WHERE tenant_id=$1 AND id=$2 FOR UPDATE",
    )
    .bind(access.tenant_id.get())
    .bind(command.mapping_id.get())
    .fetch_optional(&mut *tx)
    ...
    {
        return Err(AppError::conflict(
            "integration order owner mapping is already retired",
        ));
    }
    let revision =
        IntegrationOrderOwnerMappingRevision::new(row.try_get("revision")?).map_err(internal)?;
    if revision != command.expected_revision {
        return Err(AppError::conflict(
            "integration order owner mapping revision is stale",
        ));
    ...
    .await?;
    let result = read_owner_mapping_tx(&mut tx, access.tenant_id, command.mapping_id).await?;
    enqueue_owner_event_tx(&mut tx, &result, context.actor_id, "retired", retired_at).await?;
    Ok(prepared.commit(tx, result).await?)
}
...
pub async fn owner_page(
    db: &Db,
    access: &TenantAccess,
    query: IntegrationOrderOwnerMappingPageQuery,
) -> AppResult<IntegrationOrderOwnerMappingPage> {
    if query.limit == 0 || query.limit > 1_000 {
        return Err(AppError::bad_request(
            "integration owner mapping page limit must be between 1 and 1000",
        ));
    }
    let mut tx = begin_tenant_transaction(db, access.tenant_id).await?;
    let scope = lock_current_scope_tx(&mut tx, access.tenant_id, access.user_id.get()).await?;
    ...
    let sql = format!(
        r#"
        {OWNER_MAPPING_SELECT}
        WHERE mapping.tenant_id=$1 AND ($2 OR mapping.inventory_owner_id=ANY($3))
          AND ($4::BIGINT IS NULL OR mapping.inventory_owner_id=$4)
          AND ($5::TEXT IS NULL OR mapping.source_key=$5)
          AND (($6::TEXT IS NULL AND mapping.effective_to IS NULL)
               OR $6='active' AND mapping.effective_to IS NULL
               OR $6='retired' AND mapping.effective_to IS NOT NULL)
          AND mapping.id>$7
        ORDER BY mapping.id ASC LIMIT $8
        "#
    );
    let rows = sqlx::query(&sql)
        .bind(access.tenant_id.get())
    ...
    let next_cursor = has_more
        .then(|| items.last().map(|item| item.mapping_id))
        .flatten()
        .map(|after_mapping_id| IntegrationOrderOwnerMappingCursor { after_mapping_id });
    tx.commit().await?;
    Ok(IntegrationOrderOwnerMappingPage { items, next_cursor })
}
