...
pub async fn list(
    ...
    let accounts = repo::accounts::get_accounts(&state.db, q.show_deleted).await?;
    Ok(Json(accounts))
}
...
pub async fn add(
    ...
    validate(&body)?;
    let id = repo::accounts::add_account(&state.db, &body.name, &body.email).await?;
    Ok(Json(id))
}
...
pub async fn delete(
    ...
    user.require_permission(&state.db, PERM).await?;
    validate(&body)?;
    let ok = repo::accounts::delete_account(&state.db, body.account_id).await?;
    Ok(Json(ok))
}
...
pub async fn restore(
    ...
    user.require_permission(&state.db, PERM).await?;
    validate(&body)?;
    let ok = repo::accounts::restore_account(&state.db, body.account_id).await?;
    Ok(Json(ok))
}
