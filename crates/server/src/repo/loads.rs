...
use wareboxes_core::models::{
    ...
    LoadType, TenantAccess, Timestamp,
};
...
pub async fn update_load(
    ...
    let res = sqlx::query(
        ...
        UPDATE loads SET
            ...
            closed = COALESCE($15, closed),
            checked_in_by = COALESCE($16, checked_in_by),
            closed_by = COALESCE($17, closed_by)
        WHERE tenant_id = $18 AND id = $19 AND deleted IS NULL
        "#,
    )
