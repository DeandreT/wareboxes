...
fn ReturnDetail(
    ...
    view! { <div class="inbound-asn-detail-content"><header class="detail-heading"><div><span class="eyebrow">{format!("Return #{}",detail.summary.customer_return_id)}</span><h2>{detail.summary.number.clone()}</h2><p>{format!("Customer reference {}",detail.summary.customer_reference)}</p></div><span class=status_class(detail.summary.status)>{status_label(detail.summary.status)}</span></header><dl class="summary-grid"><div><dt>"Client"</dt><dd>{detail.summary.inventory_owner_name}</dd></div><div><dt>"Facility"</dt><dd>{detail.summary.facility_name}</dd></div><div><dt>"Load state"</dt><dd>{detail.summary.execution_status.map(execution_label).unwrap_or("Not planned")}</dd></div><div><dt>"Due"</dt><dd>{detail.summary.expected_at.as_deref().map(short_timestamp).unwrap_or_else(|| "Not supplied".into())}</dd></div><div><dt>"Authorized"</dt><dd>{format_quantity(detail.summary.total_authorized_quantity)}</dd></div><div><dt>"Quarantined"</dt><dd>{format_quantity(detail.summary.total_rejected_quantity)}</dd></div><div><dt>"Missing"</dt><dd>{format_quantity(detail.summary.total_missing_quantity)}</dd></div><div><dt>"Open"</dt><dd><strong>{format_quantity(detail.summary.total_remaining_quantity)}</strong></dd></div></dl><div class="detail-section-heading"><h3>"Authorized items"</h3><span>{format!("{} lines",detail.lines.len())}</span></div><div class="table-scroll"><table class="dense-table document-progress-table"><thead><tr><th>"Item"</th><th>"Return reason"</th><th>"Identity"</th><th>"Progress"</th></tr></thead><tbody>{detail.lines.into_iter().map(|line| view! { <tr><td><strong>{line.item_description}</strong><small>{format!("Item #{} · {}",line.item_id,line.uom)}</small></td><td><strong>{reason_label(line.reason)}</strong>{line.note.map(|note| view! { <small>{note}</small> })}</td><td>{identity_label(line.lot.as_deref(),line.serial.as_deref())}</td><td><dl class="line-metrics"><div><dt>"Authorized"</dt><dd>{format_quantity(line.authorized_quantity)}</dd></div><div><dt>"Quarantined"</dt><dd>{format_quantity(line.rejected_quantity)}</dd></div><div><dt>"Open"</dt><dd><strong>{format_quantity(line.remaining_quantity)}</strong></dd></div><div><dt>"Holds"</dt><dd>{line.inspection_hold_ids.len()}</dd></div></dl></td></tr> }).collect_view()}</tbody></table></div>{cancellation}<footer class="detail-actions"><a class="button quiet-action" href="/loads">"Open inbound load"</a>{open.then(|| view! { <button class="button danger-action" type="button" on:click=move |_| on_cancel.run(())>"Cancel return"</button> })}{open.then(|| view! { <button class="button primary-action" type="button" on:click=move |_| on_plan.run(())><Icon icon=UiIcon::Loads/><span>"Plan return load"</span></button> })}</footer></div> }
}
