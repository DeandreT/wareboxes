...
fn metrics(value: &AutomationWorkspaceResponse) -> AnyView {
    ...
    let review = value
        ...
        .count();
    view!{<section class="metric-strip"><article><span>"Devices"</span><strong>{value.devices.len()}</strong></article><article><span>"Automatic"</span><strong>{automatic}</strong></article><article><span>"Healthy / degraded"</span><strong>{healthy}</strong></article><article><span>"Active commands"</span><strong>{active}</strong></article><article><span>"Manual review"</span><strong>{review}</strong></article></section>}.into_any()
}
...
fn resolution_dialog(
    ...
    view! {
        <div class="automation-dialog-backdrop">
            <section class="automation-dialog" role="dialog" aria-modal="true">
                <header><div><span class="eyebrow">"Physical reconciliation"</span><h2>{format!("Resolve command #{}",command.command_id)}</h2></div></header>
