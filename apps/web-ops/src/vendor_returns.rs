...
pub(crate) fn VendorReturnsWorkspace(
    ...
    view! {
        <div class="vas-workspace vendor-return-workspace split-workspace" style=move ||layout.style() data-pane-mode=move ||layout.mode_attribute()>
            ...
            <section class="data-section vas-detail split-detail"><Show when=move ||signals.create_open.get() fallback=move ||view!{<Show when=move ||signals.selected.get().is_some() fallback=move ||view!{<div class="detail-empty"><h2>"Vendor return details"</h2><p>"Select a return to inspect reserved stock, outbound inventory evidence, billing, and history."</p></div>}>{move ||signals.selected.get().map(|value|view!{<ReturnDetail signals value/>})}</Show>}><CreateReturn signals access=access.get_value()/></Show></section>
        </div>
    }
}
...
fn CreateReturn(signals: Signals, access: AccessScopeWorkspace) -> impl IntoView {
    ...
    view! {<form class="vas-create" on:submit=submit><header class="detail-heading"><div><span class="eyebrow">"Controlled outbound stock"</span><h2>"New vendor return"</h2><p>"Authorize exact inventory identities before reserving or shipping them."</p></div><button class="icon-button" type="button" title="Close" aria-label="Close" on:click=move |_|signals.create_open.set(false)><Icon icon=UiIcon::Close/></button></header><div class="vas-form-grid"><label><span>"Client"</span><select prop:value=move ||option_value(owner.get()) on:change=move |event|{owner.set(parse_id(&event_target_value(&event)));lines.set(Vec::new())}><option value="">"Choose client"</option>{access.inventory_owners.into_iter().map(|item|view!{<option value=item.id>{item.name}</option>}).collect_view()}</select></label><label><span>"Facility"</span><select prop:value=move ||option_value(facility.get()) on:change=move |event|{facility.set(parse_id(&event_target_value(&event)));lines.set(Vec::new())}><option value="">"Choose facility"</option>{access.facilities.into_iter().map(|item|view!{<option value=item.id>{item.name}</option>}).collect_view()}</select></label><label><span>"Return number"</span><input prop:value=move ||number.get() on:input=move |event|number.set(event_target_value(&event)) placeholder="RTV-2026-0001"/></label><label><span>"Vendor"</span><input prop:value=move ||vendor.get() on:input=move |event|vendor.set(event_target_value(&event)) placeholder="Supplier legal or trading name"/></label><label><span>"Vendor reference"</span><input prop:value=move ||reference.get() on:input=move |event|reference.set(event_target_value(&event)) placeholder="RGA / RMA"/></label></div><label class="vas-note"><span>"Return instructions"</span><textarea prop:value=move ||note.get() on:input=move |event|note.set(event_target_value(&event))></textarea></label><section class="vas-builder"><header><h3>"Return stock"</h3><span>{move ||format!("{} lines",lines.get().len())}</span></header><div class="vas-line-editor vas-output-editor"><select prop:value=move ||option_value(balance_id.get()) on:change=move |event|balance_id.set(parse_id(&event_target_value(&event)))><option value="">"Choose available stock"</option>{options}</select><select prop:value=move ||reason_wire(reason.get()) on:change=move |event|reason.set(parse_reason(&event_target_value(&event)))><option value="defective">"Defective"</option><option value="damaged">"Damaged"</option><option value="expired">"Expired"</option><option value="recall">"Recall"</option><option value="overstock">"Overstock"</option><option value="vendor_request">"Vendor request"</option><option value="other">"Other"</option></select><input inputmode="numeric" prop:value=move ||quantity.get() on:input=move |event|quantity.set(event_target_value(&event)) placeholder="Qty"/><button class="button secondary-action compact" type="button" on:click=add>"Add"</button></div><label class="vas-note"><span>"Line evidence"</span><input prop:value=move ||line_note.get() on:input=move |event|line_note.set(event_target_value(&event)) placeholder="Failure, recall, or authorization evidence"/></label><ul class="vas-draft-lines">{move ||lines.get().into_iter().enumerate().map(|(index,line)|view!{<li><span><strong>{balance_label(&line.balance)}</strong><small>{reason_label(line.reason)}</small></span><b>{format_quantity(line.quantity)}</b><button class="icon-button" type="button" title="Remove line" aria-label="Remove line" on:click=move |_|lines.update(|values|{values.remove(index);})><Icon icon=UiIcon::Close/></button></li>}).collect_view()}</ul></section><Show when=move ||form_error.get().is_some()>{move ||form_error.get().map(|message|view!{<p class="inline-command-error">{message}</p>})}</Show><Feedback signals/><footer class="detail-actions"><button class="button quiet-action" type="button" on:click=move |_|signals.create_open.set(false)>"Discard"</button><button class="button primary-action" type="submit" disabled=move ||signals.pending.get()>"Create draft"</button></footer></form>}
}

#[component]
fn Feedback(signals: Signals) -> impl IntoView {
    view! {<Show when=move ||signals.error.get().is_some()>{move ||signals.error.get().map(|message|view!{<div class="command-recovery"><p class="inline-command-error">{message}</p><Show when=move ||signals.retry.get().is_some()><button class="button secondary-action compact" type="button" disabled=move ||signals.pending.get() on:click=move |_|{if let Some(command)=signals.retry.get_untracked(){dispatch(signals,command)}}>"Retry same command"</button></Show></div>})}</Show>}
}
