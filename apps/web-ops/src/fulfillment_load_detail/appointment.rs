...
use wareboxes_api_contract::v1::ScheduleInboundLoadRequest;
...
pub(super) fn InboundAppointmentConfirmation(
    ...
    let retry_attempt = RwSignal::new(None::<(ScheduleInboundLoadRequest, String)>);
    ...
    let submit = move |event: leptos::ev::SubmitEvent| {
        ...
        let (request, key) = if let Some(saved) = retry_attempt.get_untracked() {
            ...
            let request = ScheduleInboundLoadRequest {
        ...
        leptos::task::spawn_local(async move {
            match api::schedule_inbound_load(load_id, &request, &key).await {
                Ok(_) => {
                    retry_attempt.set(None);
                    pending.set(false);
                    on_close.run(());
                    toasts.success(format!(
                        "Appointment scheduled for inbound load #{load_id}."
                    ));
                ...
                Err(api_error) => {
                    ...
                    error.set(Some(if api_error.ambiguous_outcome {
                        "Appointment outcome is unknown. Retry to reconcile the exact saved time."
                            .to_owned()
                    } else {
                        api_error.message
                    }));
    ...
    view! {
        <form
            ...
            on:submit=submit
        >
            <h3 id="schedule-inbound-load-title">"Schedule inbound appointment"</h3>
            <p>"Set the first warehouse appointment for this planned load."</p>
            ...
            <div class="form-actions">
                <button type="submit" class="button primary-action" disabled=move || pending.get()>
                    {move || if pending.get() { "Scheduling" } else { "Schedule load" }}
                </button>
