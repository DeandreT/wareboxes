    ...
    view! {
        <div class="vas-workspace split-workspace" style=move || layout.style() data-pane-mode=move || layout.mode_attribute()>
            ...
            <section class="data-section vas-list split-master">
                ...
                <Show when=move || signals.load_error.get().is_some()>{move || signals.load_error.get().map(|message| view! { <p class="inline-command-error">{message}</p> })}</Show>
            ...
            <section class="data-section vas-detail split-detail">
                ...
                <Show when=move || signals.detail_loading.get()><div class="panel-loading">"Loading work…"</div></Show>
            </section>
        </div>
    }
}
...
fn CreateWorkPanel(signals: Signals, access: AccessScopeWorkspace) -> impl IntoView {
    ...
    view! {
        <form class="vas-create" on:submit=submit>
            ...
            <section class="vas-builder"><header><h3>"Inputs"</h3><span>{move ||format!("{} lines",inputs.get().len())}</span></header><div class="vas-line-editor"><select prop:value=move ||option_value(input_balance.get()) on:change=move |event|input_balance.set(parse_id(&event_target_value(&event)))><option value="">"Choose available stock"</option>{input_options}</select><input inputmode="numeric" prop:value=move ||input_qty.get() on:input=move |event|input_qty.set(event_target_value(&event)) placeholder="Qty"/><button class="button secondary-action compact" type="button" on:click=add_input>"Add input"</button></div><ul class="vas-draft-lines">{move ||inputs.get().into_iter().enumerate().map(|(index,line)|view!{<li><span><strong>{balance_label(&line.balance)}</strong><small>{format!("{} available",line.balance.quantity.available)}</small></span><b>{format_quantity(line.quantity)}</b><button type="button" class="icon-button" title="Remove input" aria-label="Remove input" on:click=move |_|inputs.update(|values|{values.remove(index);})><Icon icon=UiIcon::Close/></button></li>}).collect_view()}</ul></section>
            <section class="vas-builder"><header><h3>"Outputs"</h3><span>{move ||format!("{} lines",outputs.get().len())}</span></header><p class="admin-help">"Output choices reuse a visible item batch, location, and optional LPN identity. Create required master data before planning the work."</p><div class="vas-line-editor vas-output-editor"><select prop:value=move ||option_value(output_balance.get()) on:change=move |event|output_balance.set(parse_id(&event_target_value(&event)))><option value="">"Choose output identity"</option>{output_options}</select><select prop:value=move ||inventory_status_wire(output_status.get()) on:change=move |event|output_status.set(parse_inventory_status(&event_target_value(&event)))><option value="available">"Available"</option><option value="hold">"Hold"</option><option value="quarantine">"Quarantine"</option><option value="damaged">"Damaged"</option></select><input inputmode="numeric" prop:value=move ||output_qty.get() on:input=move |event|output_qty.set(event_target_value(&event)) placeholder="Qty"/><button class="button secondary-action compact" type="button" on:click=add_output>"Add output"</button></div><ul class="vas-draft-lines">{move ||outputs.get().into_iter().enumerate().map(|(index,line)|view!{<li><span><strong>{balance_label(&line.balance)}</strong><small>{inventory_status_label(line.status)}</small></span><b>{format_quantity(line.quantity)}</b><button type="button" class="icon-button" title="Remove output" aria-label="Remove output" on:click=move |_|outputs.update(|values|{values.remove(index);})><Icon icon=UiIcon::Close/></button></li>}).collect_view()}</ul></section>
            <Show when=move || signals.balances_loading.get()><p class="panel-loading">"Loading visible inventory identities…"</p></Show>
            <Show when=move ||form_error.get().is_some()>{move ||form_error.get().map(|message|view!{<p class="inline-command-error">{message}</p>})}</Show>
            <CommandFeedback signals/>
