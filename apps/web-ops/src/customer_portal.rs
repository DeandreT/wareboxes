    ...
    view! {
        <section class="customer-portal">
            <header class="portal-toolbar">
                <div class="portal-heading"><Icon icon=UiIcon::Clients/><div><h1>"Client portal"</h1><span>"Inventory, order, shipment, and document visibility"</span></div></div>
                <label><span>"Client"</span><select prop:value=move ||option_value(signals.owner_filter.get()) on:change=move |event|signals.owner_filter.set(parse_id(&event_target_value(&event)))><option value="">"All clients"</option>{move ||owner_options(signals.workspace.get())}</select></label>
                <label><span>"Facility"</span><select prop:value=move ||option_value(signals.facility_filter.get()) on:change=move |event|signals.facility_filter.set(parse_id(&event_target_value(&event)))><option value="">"All facilities"</option>{move ||facility_options(signals.workspace.get())}</select></label>
                <form class="portal-search" on:submit=move |event|{event.prevent_default();apply_search(())}><label><span>"Search"</span><input type="search" placeholder="Order, item, lot, tracking" prop:value=move ||signals.search_draft.get() on:input=move |event|signals.search_draft.set(event_target_value(&event))/></label><button class="button secondary-action compact" type="submit"><Icon icon=UiIcon::Search/>"Apply"</button></form>
                <label class="portal-history"><input type="checkbox" prop:checked=move ||signals.include_history.get() on:change=move |event|signals.include_history.set(event_target_checked(&event))/><span>"History"</span></label>
