    ...
    view! {
        <div class="purchase-order-workspace split-workspace" style=move || layout.style() data-pane-mode=move || layout.mode_attribute()>
            ...
            <section class="data-section purchase-order-detail split-detail">
                ...
                <Show when=move || detail_loading.get()><div class="panel-loading" role="status">"Loading purchase order..."</div></Show>
...
fn CreatePurchaseOrderAsnPanel(
    ...
    view! {
        <form class="purchase-order-form po-asn-form" on:submit=submit>
            ...
            <div class="fulfillment-form-grid two-column"><label><span>"ASN number"</span><input required maxlength="120" autofocus prop:value=move || number.get() on:input=move |event| number.set(event_target_value(&event))/></label><label><span>"Expected arrival"</span><input type="datetime-local" prop:value=move || expected.get() on:input=move |event| expected.set(event_target_value(&event))/></label></div>
