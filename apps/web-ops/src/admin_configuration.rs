...
fn rule_editor(draft: Draft) -> AnyView {
    match draft.kind.get() {
        ...
        DecisionRuleKind::Replenishment => view!{<div class="admin-form-grid">{number(draft.first_number,"Minimum (% of item pick-face target)",0,99)}{number(draft.second_number,"Target (% of item pick-face target)",1,100)}{flag(draft.first_flag,"Include active replenishment in projection")}</div>}.into_any(),
