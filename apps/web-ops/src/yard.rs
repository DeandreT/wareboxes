    ...
    view! {
        <section class="yard-workspace">
            ...
            {move || if signals.loading.get() && signals.workspace.get().visits.is_empty() {
                view! { <div class="yard-state"><span class="loading-line"></span><h2>"Loading yard positions"</h2></div> }.into_any()
            } else if let Some(message)=signals.load_error.get() {
                view! { <div class="yard-state error"><h2>"Yard data unavailable"</h2><p>{message}</p><button class="button secondary-action" type="button" on:click=move |_|refresh.run(())>"Try again"</button></div> }.into_any()
            } else {
...
fn command_dialog(
    ...
    view! {
        <div class="yard-dialog-backdrop" role="presentation">
            <form class="yard-dialog" on:submit=submit>
                <header><div><span class="eyebrow">"Yard control"</span><h2>{title}</h2></div><button class="icon-button" type="button" aria-label="Close dialog" on:click=move |_|signals.dialog.set(None)><Icon icon=UiIcon::Close/></button></header>
