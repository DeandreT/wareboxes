    ...
    view! {
        <div class="inbound-asn-workspace split-workspace" style=move || layout.style() data-pane-mode=move || layout.mode_attribute()>
            ...
            <section class="data-section inbound-asn-list split-master">
                ...
                <div class="table-scroll" aria-busy=move || loading.get().to_string()>
                    <table class="dense-table inbound-asn-table">
                        <caption class="sr-only">"Advance shipping notices matching the active filters"</caption>
            ...
            <section class="data-section inbound-asn-detail split-detail">
                ...
                <Show when=move || detail_loading.get()><div class="panel-loading" role="status" aria-live="polite">"Loading ASN..."</div></Show>
...
fn CancelAsnDialog(
    ...
    view! { <div class="inbound-asn-dialog-backdrop" role="presentation"><form class="inbound-asn-dialog asn-cancel-dialog" role="dialog" aria-modal="true" aria-labelledby="asn-cancel-title" on:submit=submit><header><div><span class="eyebrow">"Terminal source action"</span><h2 id="asn-cancel-title">{format!("Cancel {number}")}</h2></div><button type="button" class="icon-button" title="Close" aria-label="Close" on:click=move |_| on_close.run(())><Icon icon=UiIcon::Close/></button></header><p>"The notice remains in history, and its unreceived quantity becomes available for a replacement ASN."</p><div class="fulfillment-form-grid"><label><span>"Reason"</span><select on:change=move |event| reason.set(parse_cancellation_reason(&event_target_value(&event)))><option value="supplier_cancelled">"Supplier cancelled"</option><option value="duplicate_notice">"Duplicate notice"</option><option value="order_changed">"Purchase order changed"</option><option value="other">"Other"</option></select></label><label><span>"Note"</span><textarea maxlength="500" placeholder="Optional unless reason is Other" prop:value=move || note.get() on:input=move |event| note.set(event_target_value(&event))></textarea></label></div><Show when=move || error.get().is_some()>{move || error.get().map(|message| view! { <p class="inline-command-error" role="alert">{message}</p> })}</Show><footer><button class="button quiet-action" type="button" on:click=move |_| on_close.run(())>"Go back"</button><button class="button danger-action" type="submit" disabled=move || pending.get()>{move || if pending.get() { "Cancelling..." } else { "Cancel ASN" }}</button></footer></form></div> }
}
    ...
    view! {
        <form class="inbound-asn-form asn-create-form" on:submit=submit>
            ...
            <section class="asn-form-section" aria-labelledby="asn-source-document-title">
                ...
                <div class="fulfillment-form-grid two-column asn-document-grid">
...
fn PlanLoadDialog(
    ...
    view! { <div class="inbound-asn-dialog-backdrop" role="presentation"><form class="inbound-asn-dialog" role="dialog" aria-modal="true" aria-labelledby="asn-plan-title" on:submit=submit><header><div><span class="eyebrow">"Source-bound planning"</span><h2 id="asn-plan-title">{format!("Plan load from {}", detail.summary.number)}</h2></div><button type="button" class="icon-button" title="Close" aria-label="Close" on:click=move |_| on_close.run(())><Icon icon=UiIcon::Close/></button></header><div class="fulfillment-form-grid two-column"><label class="full-width"><span>"Receiving dock"</span><select required prop:value=move || dock.get() on:change=move |event| dock.set(event_target_value(&event))><option value="">"Choose dock"</option>{docks.into_iter().map(|location| { let label=location.name.or(location.barcode).unwrap_or_else(|| format!("Dock #{}", location.id)); view! { <option value=location.id>{label}</option> } }).collect_view()}</select></label><label><span>"Carrier"</span><input prop:value=move || carrier.get() on:input=move |event| carrier.set(event_target_value(&event))/></label><label><span>"Trailer"</span><input prop:value=move || trailer.get() on:input=move |event| trailer.set(event_target_value(&event))/></label><label><span>"Seal"</span><input prop:value=move || seal.get() on:input=move |event| seal.set(event_target_value(&event))/></label><div class="asn-plan-summary"><strong>{format_quantity(detail.summary.total_expected_quantity)}</strong><span>{format!(" units across {} source lines", detail.summary.line_count)}</span></div></div><Show when=move || error.get().is_some()>{move || error.get().map(|message| view! { <p class="inline-command-error" role="alert">{message}</p> })}</Show><footer><button class="button quiet-action" type="button" on:click=move |_| on_close.run(())>"Cancel"</button><button class="button primary-action" type="submit" disabled=move || pending.get()>{move || if pending.get() { "Planning..." } else { "Plan inbound load" }}</button></footer></form></div> }
}
