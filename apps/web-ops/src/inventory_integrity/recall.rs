...
use wareboxes_api_contract::v1::{
    CreateInventoryRecallRequest, InventoryRecallPage, InventoryRecallReason,
    InventoryRecallResponse, InventoryRecallStatus, OpaqueCursor, ReleaseInventoryRecallRequest,
};
...
impl RecallSignals {
    fn new() -> Self {
        Self {
            ...
            item_batch_id: RwSignal::new(String::new()),
...
pub(super) fn RecallView(
    ...
    target: RwSignal<Option<(i64, i64)>>,
) -> impl IntoView {
    ...
    Effect::new(move || {
        let Some((facility_id, item_batch_id)) = target.get() else {
            return;
        };
        signals.facility_id.set(facility_id.to_string());
        signals.item_batch_id.set(item_batch_id.to_string());
    ...
    let submit_create = move |event: leptos::ev::SubmitEvent| {
        ...
        let Some(facility_id) = positive_id(&signals.facility_id.get_untracked()) else {
            signals
                .error
                .set(Some("Enter a positive facility ID.".into()));
            return;
        };
        let Some(item_batch_id) = positive_id(&signals.item_batch_id.get_untracked()) else {
            signals
                .error
                .set(Some("Enter a positive item batch ID.".into()));
            return;
        };
        ...
        <section class="integrity-read-view recall-view">
            <div class="recall-control-stack">
              <div class="integrity-query-bar recall">
                ...
                <label><span>"Facility ID"</span><input inputmode="numeric" placeholder="Any" prop:value=move || signals.facility_filter.get() on:input=move |event| signals.facility_filter.set(event_target_value(&event))/></label>
                <label><span>"Client ID"</span><input inputmode="numeric" placeholder="Any" prop:value=move || signals.owner_filter.get() on:input=move |event| signals.owner_filter.set(event_target_value(&event))/></label>
            ...
            <div class="integrity-read-split split-workspace" style=move || layout.style() data-pane-mode=move || layout.mode_attribute()>
                ...
                <aside class="data-section split-detail recall-detail">
                    {move || if signals.create_open.get() {
                        view! { <RecallCreateForm signals on_submit=Callback::new(submit_create)/> }.into_any()
                    } else {
...
fn RecallCreateForm(
    ...
    view! { <form class="recall-create-form" on:submit=move |event| on_submit.run(event)>
        ...
        <div class="recall-form-grid">
            <label><span>"Facility ID"</span><input required inputmode="numeric" prop:value=move || signals.facility_id.get() on:input=move |event| {signals.facility_id.set(event_target_value(&event));signals.retry.set(None);}/></label>
            <label><span>"Item batch ID"</span><input required inputmode="numeric" prop:value=move || signals.item_batch_id.get() on:input=move |event| {signals.item_batch_id.set(event_target_value(&event));signals.retry.set(None);}/></label>
