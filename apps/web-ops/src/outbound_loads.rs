...
fn dialog_view(
    ...
    if dialog == Dialog::Plan {
        ...
        return view! { <div class="outbound-load-dialog-backdrop"><section class="outbound-load-dialog wide" role="dialog" aria-modal="true" aria-labelledby="plan-load-title"><header><div><span class="eyebrow">"Supervisor planning"</span><h2 id="plan-load-title">"Plan outbound load"</h2></div><button class="icon-button" type="button" aria-label="Close" on:click=close>"×"</button></header>
    ...
    view! { <div class="outbound-load-dialog-backdrop"><section class="outbound-load-dialog" role="dialog" aria-modal="true" aria-labelledby="load-command-title"><header><div><span class="eyebrow">"Load command"</span><h2 id="load-command-title">{title}</h2></div><button class="icon-button" type="button" aria-label="Close" on:click=close>"×"</button></header>
        ...
        <footer><button class="button quiet-action" type="button" on:click=close>"Back"</button><button class="button primary-action" type="button" disabled=move || signals.command_pending.get() on:click=move |_| submit_phase.run(dialog)>{title}</button></footer>
